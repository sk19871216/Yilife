 ...
 */
public class GetCheckNumberClient {
    public static RequestHandle requestLogin(Context context, String phone,String freshtooken, GetCheckNumberHandler responseHandler, boolean isTest) {
        GetCheckNumberRequest helper = new GetCheckNumberRequest();
        HeadInfo.Builder builder = new HeadInfo.Builder(context);
        builder.app("IEMyLife");
        builder.appID(Installation.id(context));
        helper.setHeadInfo(builder.build());
        helper.setPhone(phone);
        helper.setFreshtooken(freshtooken);


        InnovationClient client = InnovationClient.getInstance();
        client.setSSLSocketFactory();

        String path = GetCheckNumberRequest.PATH_TEST;
        if (!isTest) {
            path = GetCheckNumberRequest.PATH;
        }
